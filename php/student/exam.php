...
if(isset($_POST['finish'])){
    ...
    if(@count(@$exam) > 0) {
       ...
       $send= SubmitExam('ExamResults',$TotalScore,$myScore,$correctQuestion,$wrongQuestion,$_GET['level'],$_GET['subject'],$_SESSION['userId'],$QuestionsNumber,$missedPoints);
        if($send){
   ...
   @$degrees =  GetStudentDegrees('ExamResults',$_GET['level'],$_GET['subject'],$_SESSION['userId']);

   // $totalTime=$degrees[''];
        @$QuestionsNumber = @$degrees['questionsNumber'];
        @$TotalScore = @$degrees['totalScore'];
        @$myScore = @$degrees['score'];
        @$correctQuestion = @$degrees['correctAnswers'];
        @$wrongQuestion = @$degrees['wrongAnswers'];
        @$missedPoints = @$degrees['missedPoints'];
